...
function draw() {
	...
	text("Photographer", xDisp - fontSize + fontSize * 0.05, 1.35 * yDisp);
	text("NFT Artist", xDisp - fontSize + fontSize * 0.05, 1.5 * yDisp);
	// interactive element
	textAlign(CENTER);
	...
	for (let i = 0; i < shapeArr.length; i++) {
		...
		for (let j = 0; j < shapeArr.length; j++) {
			if (j == i) {
				...
				if (distance <= limit) {
					stroke(255);
                    let thickness = map(distance, 0, limit, 4, 0.05)
